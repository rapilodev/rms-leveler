...
const double BUFFER_DURATION = 6.0;
...
void calcWindowAmplification(struct Window* window, double loudness) {
    ...
    if (window->amplification > 1.0) window->amplification = 1.0;
}
...
static LADSPA_Descriptor c_ladspa_descriptor = { .UniqueID = 0x22b3e5, .Label = "ebur128_limiter_6s", .Name =
        "EBU R128 limit -20dBFS, stereo, 6 seconds", .Maker = "Milan Chrobok", .Copyright = "GPL 2 or 3", .PortCount = 4,
